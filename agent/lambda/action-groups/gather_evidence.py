...
def send_evidence_url(loan_id):
    print("Send Evidence URL")

    subject = "Gathering Evidence for loan ID: " + loan_id
    message = "Please upload your loan evidence in the AnyCompany Insurance Portal: " + url
...
def gather_evidence(event):
    ...
    # Extracting loanId value from event parameters
    loan_id = get_named_parameter(event, 'loan_Id')
    '''
    for param in event.get('parameters', []):
        if param.get('name') == 'loan_Id': 
            loan_id = param.get('value')
            break'''

    print("loan ID: " + str(loan_id))

    send_evidence_url(loan_id)
...
def lambda_handler(event, context):
    ...
    # API path routing
    if api_path == '/loans/{loan_Id}/gather-evidence':
